<?php /* Smarty version Smarty-3.0.8, created on 2015-01-11 14:03:35
         compiled from "/Applications/MAMP/htdocs/tpl/signup.php" */ ?>
<?php /*%%SmartyHeaderCode:20817686954b274a792bc97-39346296%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '4db0c2e64efd521fb316818b591ea95c3495c1f7' => 
    array (
      0 => '/Applications/MAMP/htdocs/tpl/signup.php',
      1 => 1389583000,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '20817686954b274a792bc97-39346296',
  'function' => 
  array (
  ),
    ...
    $('.tip').tooltip();
      ...
      function checkEmail(){
        ...
        if(!reg.test($.trim($("#email").val()))){
          ...
          $.post("<?php echo $_smarty_tpl->smarty->registered_plugins[Smarty::PLUGIN_FUNCTION]['spUrl'][0][0]->__template_spUrl(array('c'=>'cuser','a'=>'isExistEmail'),$_smarty_tpl);?>
", { email: $.trim($("#email").val())},
      ...
      function checkPhone(){
        ...
        if(!reg.test($.trim($("#phone").val()))){
          ...
          $.post("<?php echo $_smarty_tpl->smarty->registered_plugins[Smarty::PLUGIN_FUNCTION]['spUrl'][0][0]->__template_spUrl(array('c'=>'cuser','a'=>'isExistPhone'),$_smarty_tpl);?>
      ...
      $("#btn-register").click(function(){
        if($('#btn-register').attr("disabled")!="disabled"){//如果按钮是可用状态，则触发点击事件
          ...
          if(emailcheck&&phonecheck&&passwordcheck&&repasswordcheck&&namecheck&&accountcheck&&typecheck){//如果检查通过
            ...
            $.post("<?php echo $_smarty_tpl->smarty->registered_plugins[Smarty::PLUGIN_FUNCTION]['spUrl'][0][0]->__template_spUrl(array('c'=>'cuser','a'=>'register'),$_smarty_tpl);?>
", { email: $.trim($("#email").val()),phone: $.trim($("#phone").val()), password:$.trim($("#password").val()),name: $.trim($("#name").val()),account: $.trim($("#account").val()),payment: $('input[name="payment"]:checked').val(),type: $('input[name="type"]:checked').val(),invite: $.trim($("#invite-code").val().replace(/[\r\n]/g,""))},
             function(data){
               if(data){
                  $('#btn-register').attr('class',"btn btn-success disabled");
                  $('#btn-register').attr("disabled","disabled");
                  $.post("<?php echo $_smarty_tpl->smarty->registered_plugins[Smarty::PLUGIN_FUNCTION]['spUrl'][0][0]->__template_spUrl(array('c'=>'main','a'=>'email'),$_smarty_tpl);?>
",{email:$.trim($("#email").val())},function(data,status){
                    //alert("Data: " + data + "\nStatus: " + status);
                  });
